...
void Image::createTextureImage()
{
    ...
    createBuffer(m_deviceManager->GetLogicalDevice(),m_deviceManager->GetPhysicalDevice(), imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer, stagingBufferMemory);
    ...
    //image will be used as a destination in the tranfer
...
{
  vkDestroyImage(m_deviceManager->GetLogicalDevice(), textureImage, nullptr);
   vkFreeMemory(m_deviceManager->GetLogicalDevice(),textureImageMemory, nullptr);
}
...
{
    vkDestroyImageView(m_deviceManager->GetLogicalDevice(), textureImageView, nullptr);
...
void Image::createTextureSampler()
{
    ...
    if (vkCreateSampler(m_deviceManager->GetLogicalDevice(), &samplerInfo, nullptr, &textureSampler) != VK_SUCCESS) {
...
{
    VkCommandBuffer commandBuffer = beginSingleTimeCommands(m_deviceManager->GetLogicalDevice(),m_CommandPool);

    //Code that should nor be like this 
    ...
    endSingleTimeCommands(m_deviceManager->GetLogicalDevice(),m_CommandPool,m_deviceManager->GetGraphicsQueue(), commandBuffer);
}

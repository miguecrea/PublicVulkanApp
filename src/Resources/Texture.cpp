...
void Texture::GenerateMipmaps(Device* device, CommandManager* cmdManager,
    VkImage image, VkFormat format,
    int32_t width, int32_t height, uint32_t mipLevels)
{
    // Check runtime support for linear blit on this format
    VkFormatProperties props;
    vkGetPhysicalDeviceFormatProperties(device->GetPhysical(), format, &props);
    if (!(props.optimalTilingFeatures & VK_FORMAT_FEATURE_SAMPLED_IMAGE_FILTER_LINEAR_BIT))
        throw std::runtime_error("Texture format does not support linear blit for mipmaps!");
    ...
    {
        // Transition mip (i-1) from DST → SRC for blit source
        ...
        // Transition mip (i-1) to SHADER_READ_ONLY for sampling
    ...
    // Transition the last mip from DST → SHADER_READ_ONLY
